...
export class AppComponent {
  ...
  private async updateMarker() {
    ...
    this.itemList.map((item: any[]) => {
      ...
      marker.bindPopup((item: CustomMarker) => {
        return "Lade Daten...";
      });
  ...
  private centerPositionOnMap() {
    var lastCurrentPosition: Position = JSON.parse(
  ...
  public showDetails(searchResult: SearchResult) {
    ...
    let targetMarker: L.Layer = this.markerClusterGroup
      .getLayers()
      .find((layer: CustomMarker) => {
        return searchResult.id === layer.getId();
      });
    let marker = targetMarker as CustomMarker;
